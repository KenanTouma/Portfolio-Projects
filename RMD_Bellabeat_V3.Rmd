...
<center>
![ ](C:\\Users\\Kinan Touma\\Desktop\\Portfolio Projects\\Project Bellabeat\\R Files\\Bellabeat Photo.png)
</center>
...
<mark> We notice that for all three metrics, around 54% or 13 hours are considered high usage hours and around 46% or 11 hours are considered low usage hours. It is imperative that the battery life can withstand such usage on a single charge. </mark>
...
## **Key Takeaway 3:** 
<mark> By looking at the Venn Diagram, we can see that the pedometer is the most used feature overall whereas the weight logger is the least used feature. This information is important for a few reasons. If Bellabeat were to introduce a new product to their line, such as, a budget option which contains less features for a lower price, they should include the pedometer as that seems to be the most desired feature. Another important implementation of this information is how to potentially advertise the products. Bellabeat should focus on the pedometer feature more than other features in their advertising campaigns as that seems to be more important to consumers. </mark>
...
## **Key Takeaway 4:**  
<mark> By analyzing certain long term trends, Bellabeat could add a Marketplace feature into their app which would target users by recommending certain products to them based on their habits. This marketplace could include Bellabeat's own product line as well as potentially products from other brands which will be thoroughly vetted and aligned with Bellabeat's mission statement, allowing for Bellabeat to monetize their platform by taking certain percentages off of sales. </mark>

# Concluding Remarks

I would like to thank you for taking the time to go through this analysis. 
The key takeaways I would like to leave Bellabeat with are:
* A battery that can withstand 13 hours of higher usage and 11 hours of lower usage - Bellabeat could use the lower usage times to allow for charging
* A notification system that primarily acts between the hours of 8:00 AM and 9:00 PM as that is when users are most active. The notification system will serve two purposes: 1) Exercise Reminders and 2) Marketplace recommendations.
* The most desired feature in these products is the pedometer and the least desired feature is the weight logger. Bellabeat could use this information to better advertise their products as well as to know what features to focus on in future products.
* A potential implementation of a virtual marketplace where Bellabeat could sell their products as well as products from other vetted and ethical brands which support Bellabeat's mission statement. This will allow for another monetization stream. The products on the marketplace could be targetted to consumers based on their exercise habits.